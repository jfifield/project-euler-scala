...
object Problem11 extends App {
  ...
  def shiftLeft(array: Array[Int], places: Int): Array[Int] = {
    val a = Array.fill(array.length)(0)
    Array.copy(array, places, a, 0, array.length - places)
    a
  }

  def shiftRight(array: Array[Int], places: Int): Array[Int] = {
    val a = Array.fill(array.length)(0)
    Array.copy(array, 0, a, places, array.length - places)
    a
  }

  val hmax = grid.map(row => row.sliding(4).map(_.product).max).max
  val vmax = grid.sliding(4).map(_.reduceLeft((_, _).zipped.map(_ * _))).map(_.max).max
  val ldiagmax = grid.sliding(4).map(_.zipWithIndex.map(x => shiftLeft(x._1, x._2))).map(_.reduceLeft((_, _).zipped.map(_ * _))).map(_.max).max
  val rdiagmax = grid.sliding(4).map(_.zipWithIndex.map(x => shiftRight(x._1, x._2))).map(_.reduceLeft((_, _).zipped.map(_ * _))).map(_.max).max  
  println(List(hmax, vmax, ldiagmax, rdiagmax).max)
